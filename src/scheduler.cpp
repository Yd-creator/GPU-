...
vector<ScheduleRecord> GreedyScheduler::schedule() {
    ...
    while (static_cast<int>(records.size()) < static_cast<int>(jobs.size())) {
        ...
        // ========== 新增自适应碎片等待窗口核心逻辑 ==========
        vector<PendingJob> ready_jobs;          // 立即参与分配的任务
        vector<pair<PendingJob, int>> new_win;  // 新一轮窗口存储

        // 处理上一轮留在窗口内的延迟任务
        for (auto &item : delay_window) {
            PendingJob pj = item.first;
            int delay_step = item.second;

            // 兜底1：延迟步数达到上限，强制出窗口分配
            bool timeout = (delay_step >= WAIT_WINDOW_MAX_STEP);
            // 兜底2：等待时长超过当前时间1.2倍，判定饥饿，立即调度
            bool hungry = (static_cast<long double>(pj.wait_time) > 1.2L * static_cast<long double>(current_time));

            if (timeout || hungry) {
                ready_jobs.push_back(pj);
            } else {
                // 未超时且不饥饿，继续留在窗口，延迟步数+1
                new_win.emplace_back(pj, delay_step + 1);
            }
        }
        delay_window.swap(new_win);
        ...
        while (!pending_pq.empty()) {
            ...
            // 非饥饿短任务送入窗口，等待碎片填充
            delay_window.emplace_back(pj, 0);
        }

        // 构建可立即调度任务的优先级队列，保持原有动态优先级规则
        priority_queue<PendingJob, vector<PendingJob>, PendingJobCmp> ready_pq;
        for (auto &task : ready_jobs) {
            ready_pq.push(task);
        }
        // 完整4个参数，类型匹配
        tryStartPendingJobs(ready_pq, current_time, records, running_heap);
        // ======================================================
        ...
        // 计算下一个事件时间，无未来事件直接跳出防止崩溃
        long long next_t = nextEventTime(current_time, next_job_index, running_heap);
        if (next_t == 999999999999LL) {
            break;
        }
        current_time = next_t;
    }

    // 输出：遍历map避免job_id不连续导致at()越界崩溃
    vector<ScheduleRecord> ordered_records;
    ordered_records.reserve(records.size());
    for (auto &item : records) {
        ordered_records.push_back(item.second);
    }
    // 按任务ID升序排序输出
    sort(ordered_records.begin(), ordered_records.end(),
        [](const ScheduleRecord &x, const ScheduleRecord &y) {
            return x.job_id < y.job_id;
        });
...
GreedyScheduler::StartResult GreedyScheduler::tryStartOneJob(const Job &job, long long current_time) {
    const vector<pair<int, int>> &entries = feasible_machines.at(job.job_id);
    vector<tuple<double, int, vector<int>>> machine_score;

    for (auto &entry : entries) {
        int midx = entry.first;
        int gpu_cnt_need = entry.second;
        auto &m = machines[midx];
        if (!m.canEverRun(job, gpu_cnt_need)) continue;

        // 核心：挑选GPU策略——优先选已占用显存多、剩余空间刚好能放下的卡
        vector<pair<long, int>> gpu_state;
        for (int i = 0; i < m.spec.gpu_count; i++) {
            // long free_vram = m.spec.gpu_memory - m.gpu_used_vram[i];
            long free_vram = m.spec.gpu_memory - m.getGpuVram(i);
            gpu_state.emplace_back(-free_vram, i); // 负号：排序优先剩余少的卡
        }
        sort(gpu_state.begin(), gpu_state.end());

        // 选出前gpu_cnt_need张候选GPU
        vector<int> select_gpus;
        for (int i = 0; i < (int)gpu_state.size() && (int)select_gpus.size() < gpu_cnt_need; i++) {
            int gid = gpu_state[i].second;
            select_gpus.push_back(gid);
        }
        // 校验这批卡是否装得下任务显存
        if (!m.canAssignOnGpus(select_gpus, job)) {
            continue;
        }

        // 沿用你MRPM比例打分逻辑不变
        long total_vram = (long)m.spec.gpu_memory * m.spec.gpu_count;
        int total_cpu = m.spec.cpu_cores;
        long total_mem = m.spec.memory;

        long left_vram_total = 0;
        for (int gid = 0; gid < m.spec.gpu_count; gid++) {
            left_vram_total += (long)m.spec.gpu_memory - m.getGpuVram(gid);
        }
        int left_cpu = m.getRemainingCpu() - job.cpu_cores;
        long left_mem = m.getMemoryLeft() - job.memory;

        double ratio_vram = (double)left_vram_total / total_vram;
        double ratio_cpu = (double)left_cpu / total_cpu;
        double ratio_mem = (double)left_mem / total_mem;
        double ratio_score = ratio_vram + ratio_cpu + ratio_mem;

        machine_score.emplace_back(ratio_score, midx, select_gpus);
    }

    if (machine_score.empty()) return StartResult{};
    sort(machine_score.begin(), machine_score.end());

    // double best_score = get<0>(machine_score[0]);
    int best_midx = get<1>(machine_score[0]);
    std::vector<int> best_gpu_list = get<2>(machine_score[0]);
    auto &best_machine = machines[best_midx];
    auto res = best_machine.startJob(best_gpu_list, current_time, job);
    return StartResult{true, res.first, res.second};
}
